...
async function carregarInscricoes() {
  ...
  // Buscar emails dos utilizadores (estão no auth, não no profile)
  // Fazemos uma query separada para cada enrollment para obter o email
  const enriched = await Promise.all((data || []).map(async enr => {
    const { data: authUser } = await sb.auth.admin?.getUserById(enr.profiles?.id)
      .catch(() => ({ data: null })) || { data: null };
    return { ...enr, email: authUser?.user?.email || enr.profiles?.id || '—' };
  }));
