...
/** Formulário de LOGIN — usa Supabase */
  ...
  form.addEventListener('submit', async e => {
    ...
    if (error) {
      erro.textContent = error.message.includes('already')
        ? 'Este email já tem conta. Usa o Login.'
    ...
    // 2. Criar perfil na tabela profiles (caso o trigger não exista)
    if (data.user) {
...
window.addEventListener('load', async () => {
  // Verificar sessão Supabase antes de mostrar o welcome
  const { data: { session } } = await window.supabase.auth.getSession();
  ...
  // Verificar ?auth=login|signup vindo de outras páginas
